...
const address = {
  street: getEnvValue(process.env.VITE_CONTACT_ADDRESS_STREET),
  postalCode: getEnvValue(process.env.VITE_CONTACT_ADDRESS_POSTAL_CODE),
  city: getEnvValue(process.env.VITE_CONTACT_ADDRESS_CITY),
  country: getEnvValue(process.env.VITE_CONTACT_ADDRESS_COUNTRY),
};
...
export const contactInfo = {
  // E-Mail-Adresse
  email: getEnvValue(process.env.VITE_CONTACT_EMAIL),
  
  // Telefonnummer
  phone: getEnvValue(process.env.VITE_CONTACT_PHONE),
  ...
  responsiblePerson: getEnvValue(process.env.VITE_CONTACT_RESPONSIBLE_PERSON),
  
  // Domain (wird aus ENV genommen, Fallback für Entwicklung)
  get domain() {
    return (
      getEnvValue(process.env.VITE_FRONTEND_URL) ||
      getEnvValue(process.env.FRONTEND_URL) ||
      "http://localhost:8080"
    );
  },
