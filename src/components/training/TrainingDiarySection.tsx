...
import type {
  ...
  TrainingJournalMood,
  TrainingJournalSummary,
} from "@/types/training-journal";
...
export function TrainingDiarySection({
  className,
  preselectedWorkoutId,
}: TrainingDiarySectionProps) {
  const { user } = useAuth();
  ...
  const moodOptions: Array<{
    ...
    () => [
      {
        value: "energized",
        label: t("recoveryDiary.moods.energized"),
        helper: t("recoveryDiary.moods.energizedHelper"),
        emoji: "⚡",
      },
      {
        value: "balanced",
        label: t("recoveryDiary.moods.balanced"),
        helper: t("recoveryDiary.moods.balancedHelper"),
        emoji: "🙂",
      },
      {
        value: "tired",
        label: t("recoveryDiary.moods.tired"),
        helper: t("recoveryDiary.moods.tiredHelper"),
        emoji: "😴",
      },
      {
        value: "sore",
        label: t("recoveryDiary.moods.sore"),
        helper: t("recoveryDiary.moods.soreHelper"),
        emoji: "💢",
      },
      {
        value: "stressed",
        label: t("recoveryDiary.moods.stressed"),
        helper: t("recoveryDiary.moods.stressedHelper"),
        emoji: "⚠️",
      },
      {
        value: "motivated",
        label: t("recoveryDiary.moods.motivated"),
        helper: t("recoveryDiary.moods.motivatedHelper"),
        emoji: "🔥",
      },
      {
        value: "relaxed",
        label: t("recoveryDiary.moods.relaxed"),
        helper: t("recoveryDiary.moods.relaxedHelper"),
        emoji: "😌",
      },
      {
        value: "excited",
        label: t("recoveryDiary.moods.excited"),
        helper: t("recoveryDiary.moods.excitedHelper"),
        emoji: "🎉",
      },
      {
        value: "focused",
        label: t("recoveryDiary.moods.focused"),
        helper: t("recoveryDiary.moods.focusedHelper"),
        emoji: "🎯",
      },
      {
        value: "frustrated",
        label: t("recoveryDiary.moods.frustrated"),
        helper: t("recoveryDiary.moods.frustratedHelper"),
        emoji: "😤",
      },
    ],
    [t]
  ...
  const [editingEntry, setEditingEntry] = useState<TrainingJournalEntry | null>(
    null
  );
    ...
    try {
      ...
      const options: RecentWorkoutOption[] = workoutsArray
        .map((workout: Record<string, unknown>) => {
          ...
          const title =
            typeof workout.title === "string" ? workout.title : "Workout";
          const workoutDate =
  ...
  useEffect(() => {
    ...
    fetchRecentWorkouts();
  }, [user, loadEntries, fetchRecentWorkouts, loadSummary, summaryPeriod]);
  ...
  useEffect(() => {
    if (preselectedWorkoutId && preselectedWorkoutId !== "none") {
      ...
      fetchRecentWorkouts().then((loadedWorkouts) => {
        ...
        const workoutExists = loadedWorkouts.some(
  ...
  useEffect(() => {
    ...
    const loadMissingWorkouts = async () => {
      const workoutIds = entries
        .map((e) => e.workoutId)
        .filter(
          (id): id is string => id !== null && id !== undefined && id !== "none"
        )
      ...
      if (loadedWorkouts.length > 0) {
        setRecentWorkouts((prev) => {
          ...
          const newWorkouts = loadedWorkouts.filter(
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast({
        title: editingEntry
          ? t("recoveryDiary.success.entryUpdated")
          : t("recoveryDiary.success.entrySaved"),
      ...
      const message =
        error instanceof Error
          ? error.message
          : t("recoveryDiary.errors.saveErrorDescription");
      toast({
        title: t("common.error"),
        description: message,
        variant: "destructive",
  ...
  const handleEdit = async (entry: TrainingJournalEntry) => {
    ...
    setPerceivedExertion(
      entry.perceivedExertion ? String(entry.perceivedExertion) : ""
    );
    ...
    if (entry.workoutId && entry.workoutId !== "none") {
      ...
      const workoutExists = recentWorkouts.some(
    ...
    setSleepDuration(
      entry.metrics?.sleepDurationHours
        ? String(entry.metrics.sleepDurationHours)
        : ""
    );
    setRestingHeartRate(
      entry.metrics?.restingHeartRate
        ? String(entry.metrics.restingHeartRate)
        : ""
    );
    setHydrationLevel(
      entry.metrics?.hydrationLevel ? String(entry.metrics.hydrationLevel) : ""
    );
    ...
    try {
      ...
      if (!response.ok) {
        const errorBody = await response
          ...
          .catch(() => ({ error: t("common.error") }));
        throw new Error(
          errorBody.error ?? t("recoveryDiary.errors.deleteErrorDescription")
        );
      }
      ...
      const message =
        error instanceof Error
          ? error.message
          : t("recoveryDiary.errors.deleteErrorDescription");
      toast({
        title: t("common.error"),
        description: message,
        variant: "destructive",
        ...
        <CardContent className="space-y-4 pt-4">
          ...
          {summary && (
            <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-3 xl:grid-cols-6">
              ...
              <div className="rounded-lg border bg-muted/50 p-3">
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
                  {t("recoveryDiary.avgExertion")}
                </p>
                <p className="text-lg font-semibold">
                  {formatScaleValue(summary.avgPerceivedExertion)}
                </p>
              </div>
            </div>
          )}
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {entries.map((entry) => {
                ...
                const entryWorkout = entry.workoutId
                  ? recentWorkouts.find((w) => w.id === entry.workoutId)
                  : null;
                return (
                  <div
                    key={entry.id}
                      ...
                      <div className="space-y-2 flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {entry.workoutId ? (
                            <Badge
                              variant="outline"
                              className="text-xs flex flex-row items-center justify-between gap-2 px-2 py-1"
                            >
                              <span className="font-medium">
                                {entryWorkout
                                  ? entryWorkout.title
                        ...
                        <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                          <span>
                            {t("recoveryDiary.metrics.energy")}:{" "}
                            {formatScaleValue(entry.energyLevel)}
                          </span>
                          <span>
                            {t("recoveryDiary.metrics.focus")}:{" "}
                            {formatScaleValue(entry.focusLevel)}
                          </span>
                          <span>
                            {t("recoveryDiary.metrics.sleep")}:{" "}
                            {formatScaleValue(entry.sleepQuality)}
                          </span>
                          <span>
                            {t("recoveryDiary.metrics.soreness")}:{" "}
                            {formatScaleValue(entry.sorenessLevel)}
                          </span>
                          <span>
                            {t("recoveryDiary.metrics.exertion")}:{" "}
                            {formatScaleValue(entry.perceivedExertion)}
                          </span>
