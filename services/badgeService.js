...
export const badgeService = {
  ...
  async handleLifetimeMilestones(pool, userId, activityType, totalQuantity) {
    await ensureBadgeCatalog(pool);
    const definition = LIFETIME_ACTIVITY_BADGES.find(
    ...
    for (const threshold of definition.thresholds) {
      if (totalQuantity >= threshold) {
        const badge = await getBadgeBySlugAndLevel(
          pool,
          definition.slug,
          threshold
        );
  ...
  async grantCustomBadge(
    pool,
    userId,
    { slug, label, description, icon, level = null, context = {} }
  ) {
    const { rows } = await pool.query(
