...
export const createProfileRouter = (pool) => {
  ...
  router.post("/change-password", authMiddleware, async (req, res) => {
    try {
      ...
      if (!isMatch) {
        return res
          .status(401)
          .json({ error: "Ungültiges aktuelles Passwort." });
      }
      ...
      res
        .status(500)
        .json({ error: "Serverfehler beim Ändern des Passworts." });
    }
  });
  ...
  router.get("/friends/:friendId", authMiddleware, async (req, res) => {
    try {
      ...
      if (friendId === req.user.id) {
        return res
          .status(400)
          .json({ error: "Nutze dein eigenes Profil im Profilbereich." });
      }
      ...
      if (columnNames.length === 0) {
        return res
          .status(400)
          .json({ error: "Freundesfunktionen sind nicht aktiviert." });
      }
      ...
      const { rowCount: friendshipExists } = await pool.query(friendshipQuery, [
        req.user.id,
        friendId,
      ]);
      ...
      const recentWorkouts = workoutRows.map((row) => ({
        ...
        activities: (row.activities || []).filter((a) => a.id !== null),
      ...
      res
        .status(500)
        .json({ error: "Serverfehler beim Laden des Freundesprofils." });
    }
  });

  // GET /api/profile/achievements - Get user's badges and awards
  router.get("/achievements", authMiddleware, async (req, res) => {
    try {
      ...
      // Load badges
      const { rows: badgeRows } = await pool.query(
        `SELECT ub.id, b.slug, b.label, b.description, b.icon, b.category, b.level, ub.earned_at
                 FROM user_badges ub
                 JOIN badges b ON b.id = ub.badge_id
                 WHERE ub.user_id = $1
                 ORDER BY ub.earned_at DESC`,
      ...
      res.status(500).json({ error: "Serverfehler beim Laden der Erfolge." });
    }
  });
    ...
    try {
      ...
      if (existingUsers.length > 0) {
        ...
        if (existingFriends > 0) {
          return res
            .status(409)
            .json({ error: "Ihr seid bereits befreundet." });
        }
        ...
        const displayName =
          targetUser.display_preference === "nickname" && targetUser.nickname
            ? targetUser.nickname
      ...
      const invitations = rows.map((row) => {
        ...
        return {
          ...
          expiresAt:
            ...
            new Date().toISOString(),
          used:
            invitation.used !== undefined ? Boolean(invitation.used) : false,
