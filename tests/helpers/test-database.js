...
const SCHEMA_TABLES = {
  ...
  workout_activities: ['id', 'workout_id', 'activity_type', 'quantity', 'created_at'],
};
...
export class TestDatabase {
  ...
  insertFriendship({
    ...
    const ordered = [resolvedRequester, resolvedAddressee].filter(Boolean).sort();
  ...
  insertWorkout({ id = randomUUID(), userId, title, workoutDate = new Date(), startTime, duration = null }) {
  ...
  insertWorkoutActivity({ id = randomUUID(), workoutId, activityType, quantity, pointsEarned = 0, createdAt = new Date() }) {
    const activity = {
      id,
  ...
  async query(sql, params = []) {
    ...
    if (normalized.startsWith('select column_name from information_schema.columns')) {
      ...
      const columns = tableName && SCHEMA_TABLES[tableName] ? SCHEMA_TABLES[tableName] : [];
      return {
    ...
    if (normalized.startsWith('select 1 from friendships where ((requester_id = $1 and addressee_id = $2) or (requester_id = $2 and addressee_id = $1))')) {
      const [a, b] = params;
      const requireAccepted = normalized.includes("status = 'accepted'");
      const exists = this.friendships.some((f) => {
        const match = (f.requester_id === a && f.addressee_id === b) || (f.requester_id === b && f.addressee_id === a);
    ...
    if (normalized.startsWith('insert into friendships (id, requester_id, addressee_id, status)')) {
      ...
      if (existing) {
        ...
        this.insertFriendship({ id, requesterId, addresseeId, status: 'accepted' });
      }
      return { rows: [], rowCount: 1 };
    }

    if (normalized.startsWith('select id, requester_id, addressee_id from friendships where id = $1')) {
    ...
    if (normalized.startsWith('select f.id as friendship_id') && normalized.includes('case when f.requester_id = $1')) {
    ...
    if (normalized.startsWith('select case when requester_id = $1 then addressee_id else requester_id end as friend_id from friendships')) {
      const userId = params[0];
      const rows = this.friendships
        .filter((f) => (f.requester_id === userId || f.addressee_id === userId) && f.status === 'accepted')
        .map((f) => ({
    ...
    if (normalized.startsWith('select id, email, first_name, last_name, nickname, display_preference, avatar_url from users where (')) {
      const [searchPattern, currentUserId, exactPattern, limit, offset] = params;
