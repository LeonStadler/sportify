...
describe('Friends API', () => {
  it('handles friend request lifecycle end-to-end', async () => {
    const alice = db.insertUser({ email: 'alice@example.com', firstName: 'Alice', lastName: 'Anderson' });
    const bob = db.insertUser({ email: 'bob@example.com', firstName: 'Bob', lastName: 'Brown' });
    ...
    const bobRequestsResponse = await authFetch('/api/friends/requests', { token: bobToken });
    ...
    const aliceRequestsResponse = await authFetch('/api/friends/requests', { token: aliceToken });
    ...
    const acceptResponse = await authFetch(`/api/friends/requests/${requestId}`, {
      method: 'PUT',
      token: bobToken,
      body: { action: 'accept' },
    });
    ...
    const friendsResponse = await authFetch('/api/friends', { token: aliceToken });
    ...
    const bobFriendsResponse = await authFetch('/api/friends', { token: bobToken });
  ...
  it('prevents unauthorized acceptance and supports declining requests', async () => {
    const charlie = db.insertUser({ email: 'charlie@example.com', firstName: 'Charlie', lastName: 'Clark' });
    const diana = db.insertUser({ email: 'diana@example.com', firstName: 'Diana', lastName: 'Doe' });
    ...
    const invalidAccept = await authFetch(`/api/friends/requests/${requestId}`, {
      method: 'PUT',
      token: charlieToken,
      body: { action: 'accept' },
    });
    ...
    const declineResponse = await authFetch(`/api/friends/requests/${requestId}`, {
      method: 'PUT',
      token: dianaToken,
      body: { action: 'decline' },
    });
    ...
    const friendsResponse = await authFetch('/api/friends', { token: charlieToken });
    const friends = await friendsResponse.json();
...
describe('User search API', () => {
  it('searches case-insensitively and excludes the current user', async () => {
    const alpha = db.insertUser({ email: 'alpha@example.com', firstName: 'Alice', lastName: 'Zenith' });
    const bravo = db.insertUser({ email: 'bravo@example.com', firstName: 'Bob', lastName: 'Allan', nickname: 'Bobby' });
    const charlie = db.insertUser({ email: 'charlie@example.com', firstName: 'Carla', lastName: 'Stone' });

    const token = signToken(alpha.id);

    const searchResponse = await authFetch('/api/users/search?query=bo', { token });
    ...
    const caseInsensitiveResponse = await authFetch('/api/users/search?query=STO', { token });
...
describe('Activity feed', () => {
  it('returns activities from the user and their friends only', async () => {
    const user = db.insertUser({ email: 'user@example.com', firstName: 'Una', lastName: 'User' });
    const friend = db.insertUser({ email: 'friend@example.com', firstName: 'Frank', lastName: 'Friend' });
    const stranger = db.insertUser({ email: 'stranger@example.com', firstName: 'Sam', lastName: 'Stranger' });
    ...
    const displayNames = feed.activities.map((activity) => activity.displayName);
    assert(displayNames.includes('Una'));
    assert(displayNames.includes('Frank'));
    assert(!displayNames.includes('Sam'));
    assert.equal(feed.activities[0].displayName, 'Frank');
  });
});
